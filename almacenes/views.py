...
def entregar_orden(request,orden_id):
    ...
    cant_productos = Orden_Producto.objects.filter(orden=orden)
    ...
    ProductoFormSet = modelformset_factory(Producto,form=stockForm, extra=0,max_num=cant_productos.count())
    if request.method == 'POST':
